...
from tools.utils.variables import data_folder
...
def filter_by_score(data, filt_by, filt_thres, dataset_fbase):
    ...
    scores_fpath = Path(data_folder) / 'conservation_and_distance' / f'{dataset_fbase}_{filt_by}.csv'
    scores = pd.read_csv(scores_fpath)
    if filt_by == 'sift':
        filt_by += '_avg'
