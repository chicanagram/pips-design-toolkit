...
def main():
    ...
    from conservation_and_distance.filter_data import filter_by_score
    ...
    # other settings
    load_model = f'{data_folder}trained_models/{dataset_fbase}/' # set to model directory if loading a pre-trained model
    ...
    # get data
    dataset_fpath_all = f'{data_folder}Input/{dataset_fbase}_test.csv'
    if filt_by=='':
        XY_test = pd.read_csv(dataset_fpath_all)[x_features + [y_feature]]
    else:
        XY_test = pd.read_csv(dataset_fpath_all)[x_features + [y_feature, 'Position']]
    ...
    if filt_by!='':
        ...
        XY_test, _ = filter_by_score(XY_test, filt_by, filt_thres, dataset_fbase_0)

    # convert to autogluon tabular dataset
    test_data = TabularDataset(XY_test)

   # train and validate model #
    predictor, y_pred, res = autogluon_classifier(
        None, test_data, label=y_feature, metrics=eval_metric_list,
