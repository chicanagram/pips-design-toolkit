...
from utils.utils import fetch_sequences_from_fasta, write_sequence_to_fasta, get_mutations, get_mutated_sequence, mkDir

def AggWaltz(mutations, seq_base, seq_name, inPath, outPath, output_csv_fname):
    print('CWD:', os.getcwd())
    waltz_path = "./feature_extraction/aggregation/Waltz.pl"
    ...
    # add directory to store results if not present
    if not os.path.exists(outPath + 'waltz/'):
        new_dir = mkDir('waltz', outPath, remove_existing_dir=False)
...
def AggTango(mutations, seq_base, seq_name, inPath, outPath, output_csv_fname):
    ...
    tango_path = os.path.abspath('./feature_extraction/aggregation/Tango.exe')
    ...
    # add directory to store results if not present
    if not os.path.exists(outPath + 'tango/'):
        new_dir = mkDir('tango', outPath, remove_existing_dir=False)
...
if __name__ == '__main__':
    ...
    inPath = data_folder + 'feature_extraction/Input/'
    outPath = data_folder + 'feature_extraction/'
    sequence_fname = 'GOh1052'

    # specify base sequence(s)
    sequence_fpath = data_folder + 'sequences/' + sequence_fname + '.fasta'
    seq_base, seq_name, seq_description = fetch_sequences_from_fasta(sequence_fpath)
    output_csv_fname = f'AggregationScore_{sequence_fname}'
    ...
    if 'waltz' in waltz_or_tango:
        AggWaltz(mutations, seq_base, seq_name, inPath+'waltz/', outPath, output_csv_fname)
    ## RUN TANGO ##
    if 'tango' in waltz_or_tango:
        AggTango(mutations, seq_base, seq_name, inPath+'tango/', outPath, output_csv_fname)
