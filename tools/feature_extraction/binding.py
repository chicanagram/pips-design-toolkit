...
def mutate_residue(
    ...
    log_fpath = output_dir + output_fname
...
def get_yasara_binding_features(
    ...
    print('mutations by position:', res_mut_dict)

    proc_num = 0
    for i, mutation in enumerate(mutations):
        mutstr, mutation = get_mutstr(mutation)
        mutation = backbone_mutations + mutation
        mutate_residue(mutation, struct_fname, output_fname, struct_dir, output_dir, move='!backbone', minimize_energy=True, resetSce=False, nrep=nrep)
        proc_num += 1
...
if __name__ == "__main__":
    ...
    nrep = 5
    backbone_mutations = ['F194A','N245W'] # [] empty list if no backbone mutations to add
