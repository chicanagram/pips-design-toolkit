...
class PLMZero:
    def __init__(
        ...
        self.conservation_and_distance_dir = data_folder + subfolders['conservation_and_distance'] + data_subfolder + '/'
        print('Embeddings dir:', self.embeddings_dir)
        print('Conservation analysis dir:', self.conservation_and_distance_dir)
        mkDir(data_subfolder, data_folder + subfolders['conservation_and_distance'], remove_existing_dir=False)
    ...
    def get_inputs(
        ...
        self.llrsum_entropy_mutants_fpath = self.conservation_and_distance_dir + fname + '_LLRsum_entropy.csv'
    ...
    def parse_plm_results(self, inputs_all, get_llr_entropy_plot=False):
        ...
        for i, (seq_name, seq_base, seq_mut, mut) in enumerate(zip(inputs_all['base']['name'], inputs_all['base']['sequence'], inputs_all['mut']['sequence'], inputs_all['mut']['mutations'])):
            ...
            if self.get_mutation_logits_probs is not None and seq_base not in seq_base_processed:
                ...
                mutprobs_fpath = f'{self.conservation_and_distance_dir}{self.model_type}-{self.layer_str}/{seq_name}_{self.model_type}-{self.layer_str}_MutProbs.csv'
                ...
                savefig = f'{self.conservation_and_distance_dir}{self.model_type}-{self.layer_str}/{seq_name}_{self.model_type}-{self.layer_str}'
                ...
                # save as CSV
                llr_entropy_seqbase.to_csv(f'{self.conservation_and_distance_dir}{self.model_type}-{self.layer_str}/{seq_name}_{self.model_type}-{self.layer_str}_LLR_entropy.csv')
